...
void nanoappHandleEvent(uint32_t senderInstanceId, uint16_t eventType,
  ...
  chre::cross_validator_sensor::ManagerSingleton::get()->handleEvent(
      senderInstanceId, eventType, eventData);
}

bool nanoappStart(void) {
  chre::cross_validator_sensor::ManagerSingleton::init();
  return true;
}

void nanoappEnd(void) {
  chre::cross_validator_sensor::ManagerSingleton::deinit();
}
