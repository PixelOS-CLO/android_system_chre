...
/**
 * A simulated implementation of the Sensor PAL for the CHRE AP platform.
 */
namespace {
...
bool gIsApiClose = false;
bool gIsSensor0Enabled = false;
uint64_t gSensoe0IntervalNs = 1000000000;  // 10s default interval
uint64_t gCheckIntervalNs = 1000000000;    // 1s default check interval
std::thread *gSensor0Thread = nullptr;
...
bool chrePalSensorApiOpen(const struct chrePalSystemApi *systemApi,
  ...
  gSensor0Thread = new std::thread([] {
    while (!gIsApiClose) {
      if (gIsSensor0Enabled) {
        sendSensor0Events();
        std::this_thread::sleep_for(
            std::chrono::nanoseconds(gSensoe0IntervalNs));
      } else {
        std::this_thread::sleep_for(std::chrono::nanoseconds(gCheckIntervalNs));
      }
    }
  });
