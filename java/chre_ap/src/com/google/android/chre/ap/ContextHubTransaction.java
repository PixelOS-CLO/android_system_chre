...
/**
 * A class describing a request sent to the ContextHub AP.
 *
 * <p>This object is generated as a result of an asynchronous request sent to the Context Hub
 * through the ContextHubManager APIs. The caller can either retrieve the result synchronously
 * through a blocking call ({@link #waitForResponse(long, TimeUnit)}) or asynchronously through a
 * user-defined listener ({@link #setOnCompleteListener(OnCompleteListener, Executor)} )}).
 *
...
public class ContextHubTransaction<T> {
    ...
    /*
     * true if the response has been set through setResponse, false otherwise.
     */
     ...
     *
     * <p>This function blocks until the ContextHub AP has received a response for the
     * transaction represented by this object by the Context Hub, or a specified timeout period has
     * elapsed.
     *
     * <p>If the specified timeout has passed, a TimeoutException will be thrown and the caller may
     ...
     *
     * <p>This function provides an asynchronous approach to retrieve the result of the
     * transaction.
     * When the transaction response has been provided by the Context Hub, the given listener will
     * be
     * invoked.
     *
     * <p>If the transaction has already completed at the time of invocation, the listener will be
     * immediately invoked. If the transaction has been invalidated, the listener will never be
     * invoked.
     *
     * <p>A transaction can be invalidated if the process owning the transaction is no longer
     * active
     * and the reference to this object is lost.
     *
     * <p>This method or {@link #setOnCompleteListener(ContextHubTransaction.OnCompleteListener)}
     * can
     ...
     * Sets the listener to be invoked invoked when the transaction completes.
     *
     * <p>Equivalent to {@link #setOnCompleteListener(ContextHubTransaction.OnCompleteListener,
     ...
     *
     * <p>This method should only be invoked by ContextHubAPManager as a result of a callback from
     * the ContextHub AP indicating the response from a transaction. This method should not be
